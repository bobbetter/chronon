...
/** SerDe for the returns events when messages are simple JSON objects with fields:
  ...
  */
class ReturnsSerDe(topicInfo: TopicInfo) extends SerDe {

  private val zSchema: ZStructType = ZStructType(
    "returns_event",
    Array(
      ZStructField("ts", ZLongType),
      ZStructField("return_id", ZStringType),
      ZStructField("user_id", ZStringType),
      ZStructField("product_id", ZStringType),
      ZStructField("refund_amt", ZDoubleType)
    )
  )
  ...
  override def fromBytes(bytes: Array[Byte]): Mutation = {
    ...
    val row: Array[Any] = Array[Any](
      ...
      toStringOrNull(parsed.get("return_id")),
      toStringOrNull(parsed.get("user_id")),
      toStringOrNull(parsed.get("product_id")),
      toDoubleOrNull(parsed.get("refund_amt"))
    )
