...
def list_tables(db_name: str, scanner: DataScanner = Depends(get_datascanner)):
    ...
    try:
        ...
        logger.error(f"Error listing tables in database '{db_name}': {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to list tables: {str(e)}")


@router.get("/databases/{db_name}/tables/{table_name}/stats", response_model=TableStatsResponse)
def get_table_stats(db_name: str, table_name: str, scanner: DataScanner = Depends(get_datascanner)):
    """
    ...
    try:
        ...
        logger.info(f"Retrieved stats for {db_name}.{table_name}: {stats['row_count']} rows, {stats['column_count']} columns")
        ...
        logger.error(f"Error getting stats for {db_name}.{table_name}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get table stats: {str(e)}")


@router.get("/databases/{db_name}/tables/{table_name}/sample", response_model=TableSampleResponse)
def sample_table(
    db_name: str,
    table_name: str,
...
@router.get("/databases/{db_name}/tables/{table_name}/schema", response_model=List[ColumnSchema])
def get_table_schema(db_name: str, table_name: str, scanner: DataScanner = Depends(get_datascanner)):
    """
    ...
    try:
        ...
        logger.info(f"Retrieved schema for {db_name}.{table_name}: {len(schema)} columns")
        return schema
    except Exception as e:
        logger.error(f"Error getting schema for {db_name}.{table_name}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get table schema: {str(e)}")


@router.post("/query", response_model=QueryResponse)
def execute_query(request: QueryRequest, scanner: DataScanner = Depends(get_datascanner)):
    """
