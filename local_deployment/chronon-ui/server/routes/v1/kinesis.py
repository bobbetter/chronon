...
class PutRecordsResponse(BaseModel):
    ...
    records_pushed: Optional[int] = Field(
        None, description="Number of records successfully pushed"
    )
...
class GenerateTestRecordsRequest(BaseModel):
    ...
    count: int = Field(
        10, description="Number of test records to generate", example=10, gt=0, le=500
    )
...
class ClearStreamRequest(BaseModel):
    ...
    enforce_consumer_deletion: bool = Field(
        False, description="Force deletion even if consumers are registered"
    )
...
async def list_streams(limit: int = 100):
    """
    ...
    try:
        ...
        return ListStreamsResponse(streams=streams, stream_count=len(streams))
...
async def generate_and_put_test_records(
    stream_name: str, request: GenerateTestRecordsRequest
):
    """
    ...
    try:
        logger.info(
            f"Generating and putting {request.count} test records to stream '{stream_name}'"
        )
...
async def get_stream_summary(stream_name: str):
    ...
    try:
        ...
        return StreamSummaryResponse(stream_name=stream_name, summary=summary)
    except Exception as e:
...
async def list_shards(stream_name: str):
    ...
    try:
        ...
        return ShardsResponse(
            stream_name=stream_name, shards=shards, shard_count=len(shards)
        )
    except Exception as e:
...
async def read_records(
    stream_name: str, iterator_type: ShardIteratorType, limit: int = 10
):
    """
    ...
    try:
        ...
        records = kinesis_client.read_records(
            stream_name=stream_name, iterator_type=iterator_type.value, limit=limit
        )
...
async def create_stream(stream_name: str, request: CreateStreamRequest):
    ...
    try:
        logger.info(
            f"Creating stream '{stream_name}' with {request.shard_count} shard(s)"
        )
        ...
        if "ResourceInUseException" in error_msg:
            raise HTTPException(
                status_code=409, detail=f"Stream '{stream_name}' already exists"
            )

        raise HTTPException(
            status_code=500, detail=f"Failed to create stream: {error_msg}"
        )
...
async def delete_stream(
    ...
    try:
        logger.info(
            f"Deleting stream '{stream_name}' (enforce_consumer_deletion={enforce_consumer_deletion})"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete stream: {error_msg}"
        )
...
async def clear_stream_records(stream_name: str, request: ClearStreamRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to clear stream: {error_msg}"
        )
